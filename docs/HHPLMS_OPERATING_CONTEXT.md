...
### Pending application issues — δεν διορθώθηκαν σε αυτό το task
...
Τοπικά GREEN: πραγματικός Worker/page handler και signed-token verifier απορρίπτουν missing/malformed/expired/wrong-scope tokens και cookie fallback· τέσσερα populated B1/B1+ SB/WB browser fixtures ελέγχουν πραγματικά image requests, ready state, άνοιγμα σελίδας, component switching και Workbook failure/retry με νέο context. Regression sources: `tests/managed-page-startup-authorization.test.js`, `scripts/book-builder/managed-saved-draft-{fixtures,startup-playwright}.mjs`, μέσω του υπάρχοντος CI harness `scripts/book-builder/multi-book-component-playwright.mjs`. Τα assets/δεδομένα είναι synthetic και τα browser URLs πλήρως intercepted. **Hosted/staging acceptance: NOT RUN**· δεν έγινε deployment ούτε αποδεικνύεται ότι το live Saved Draft έχει ήδη διορθωθεί. Το published custom-UI Fix B παραμένει pending και εκτός αυτής της αλλαγής.

**Published custom UI:** ο χρήστης βλέπει legacy B2 UI στα B1/B1+. Η προηγούμενη repository διάγνωση εντόπισε κενό managed publication UI snapshot. Πριν από fix επανελέγχονται οι actual compiler/projection/provider διαδρομές στο νέο current SHA και αποδεικνύεται το αντίστοιχο regression. Δεν αλλάζουμε ιστορικά immutable releases ούτε κάνουμε αυτόματο republish.
...
**Hosted B1/B1+ acceptance:** publication είναι user-reported και το B1 status έχει partial concrete header evidence, αλλά πλήρες sanitized B1/B1+ publication receipt δεν υπάρχει. Τα Saved Draft 401 και published custom-UI issues παραμένουν pending application bugs και δεν διορθώθηκαν στο context work.
