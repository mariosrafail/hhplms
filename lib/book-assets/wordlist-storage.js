...
export class WordListR2Storage extends CloudflareR2ReleaseStorage {
  ...
  async download({ profile, objectKey }) {
    if (profile === "public") {
      if (!/^publishers\/hamilton-house\/books\/ultimate-b2\/editions\/students-book\/versions\/hosted-draft\/components\/ultimate-b2-students-book\/teacher-ui\/assets\/[a-f0-9]{64}\.(png|jpg|webp|mp3|wav|gaf)$/.test(objectKey)) reject("wordlist_ui_asset_scope");
