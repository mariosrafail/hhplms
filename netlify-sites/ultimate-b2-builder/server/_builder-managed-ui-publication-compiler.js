...
export function compileManagedUiReleaseV2(sources, componentSlug) {
  ...
  const compiled = { ...base, ...identity(componentSlug), compatibility: compatibility(componentSlug, base.compatibility),
    ...
    assetManifest: [...base.assetManifest, ...uiAssets].sort(order) };
...
export function verifyManagedUiReleaseV2(release, componentSlug) {
  ...
  if (sourceSnapshot.teacherUi.revision === 0 && (uiAssets.length || sourceSnapshot.teacherUi.sha256 !== hash(createEmptyBuilderTeacherUiDocument(componentSlug)))) fail();
  ...
  const privateManifest = release.asset_manifest.filter((asset) => asset.role !== "teacher_ui");
  if (json(release.asset_manifest) !== json([...privateManifest, ...uiAssets].sort(order))) fail();
