...
export async function readPublishedEdition(sql, currentUser, query, overrides = {}) {
  const deps = { ready: editionDatabaseReady, allowed: editionUserAllowed, load: loadEditionRelease,
  ...
  try {
    ...
    return await editionReleaseRead(release, query, query.assetSha256 || query.teacherAssetActivityId ? deps.storage() : null, { teacher });
  } catch (error) {
