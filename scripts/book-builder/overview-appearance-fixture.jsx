...
window.renderOverview = ({ bookSlug = "ultimate-b1", component = "students-book", dense = false, grouped = false, ui = null, runtimeContext = { kind: "builder-preview" } } = {}) => {
  ...
  if (grouped && !canonical) unit.pages = [60, 61, 62, 64].map((number, index) => ({
    id: `${bookSlug}-${component}-page-${number}`, overviewLabel: index < 2 ? "Practice 5" : "Reading",
    printedLabel: index === 2 ? "pg 62-63" : `pg ${number}`, imageWidth: index === 2 ? 1180 : 581,
    imageHeight: 794, images: [image(index === 2)],
  }));
  window.overviewPageIds = unit.pages.map((page) => page.id);
  window.selectionEvents = [];
  const identity = { bookSlug, componentSlug: `${bookSlug}-${component}` };
  const assets = createUltimateB2TeacherRuntimeUiAssets(ui, runtimeContext, { bookSlug, componentSlug: `${bookSlug}-students-book` });
  ...
  root.render(<TeacherRuntimeUiAssetsProvider value={assets}><ClassroomToolsProvider><TeacherFixedStage viewport={viewport}><main className="teacher-offline-settings-surface" data-teacher-theme="modern"><div className="teacher-offline-book unit-overview-active" style={{ height: "100%", "--legacy-classroom-background": `url(${assets.classroom.backgrounds.classroomGlacier})`, "--legacy-students-book-parts-background": `url(${assets.classroom.backgrounds.studentsBookPartsBackground})` }}><TeacherOfflineUnitOverview unit={unit} selectedBookId={component} componentIdentity={identity} onSelectPage={(id) => { window.selectedPage = id; window.selectionEvents.push(id); }} /></div></main></TeacherFixedStage></ClassroomToolsProvider></TeacherRuntimeUiAssetsProvider>);
};
