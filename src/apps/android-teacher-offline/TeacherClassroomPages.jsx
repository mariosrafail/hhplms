...
import { usePageWordList } from "../../components/wordlists/usePageWordList.js";
import { WordListOverlay } from "../../components/wordlists/WordListOverlay.jsx";
...
export function TeacherClassroomPages({
  unit,
  selectedPageId,
  onSelectPage,
  activeActivity,
  ...
  const wordlist = usePageWordList({ provider: classroom?.wordlist, pageIds: page ? classroom?.selectedPageIds || [page.id] : [], surface: activityActive ? "activity" : page ? "page" : "overview" });
  useEffect(() => { classroom?.onWordListOpenChange?.(wordlist.open); }, [classroom?.onWordListOpenChange, wordlist.open]);
  ...
  const wordListReason = { loading: "Loading Word List…", empty: "No Word List entries are mapped to this page.", unavailable: "Word List unavailable for this source.", error: "Word List could not load." }[wordlist.state];
  const contextActions = [...revealActions, ...standardContextActions, ...(wordlist.supported ? [{ id: "vocabulary", controlId: "navigation:vocabulary", label: "Vocabulary", title: wordListReason || "Word List", active: wordlist.open, disabled: wordlist.state !== "ready", artwork: legacyClassroomAssets.vocabulary, onClick: wordlist.toggle }] : [])];
  ...
  if (!page) return (
    <TeacherOfflineUnitOverview
      verifiedSource={Boolean(classroom)}
  ...
  return (
    <section ref={viewerRef} className="teacher-offline-pages teacher-offline-pages-viewer" data-wordlist-frame="" data-native-media-scope="">
      ...
      <div className={`teacher-offline-page-reader ${activityActive ? "has-embedded-activity" : ""}`.trim()}>
        ...
        >
          ...
          {activityActive ? (
            <ActivityRenderer
              ...
              classroom={classroom}
            />
          ) : classroom?.renderSpread && classroom.selectedPageIds.length === 2 ? classroom.renderSpread(onOpenActivity) : image && !assetError ? (
            <div
        ...
        {wordlist.wordlist && <WordListOverlay key={wordlist.key} model={wordlist} frameRef={viewerRef} subtitle={`${selectedBookId === "workbook" ? "Workbook" : "Students Book"} · ${classroom?.pageSubtitle || page.label || page.title}`} />}
        {!activityActive && (pageExtraVideos.length || fallbackExtraVideoActions.length) ? <div className="teacher-unit-extra-videos" inert={wordlist.open}>
        ...
        {activeExtraVideo ? <div className="teacher-unit-extra-overlay" inert={wordlist.open} role="dialog" aria-modal="true" aria-labelledby="teacher-unit-extra-title" onPointerDown={(event) => { if (event.target === event.currentTarget) { setActiveExtraVideo(null); extraLauncherRef.current?.focus(); } }}><section><header><div><span>Extra Videos</span><h2 id="teacher-unit-extra-title">{activeExtraVideo.title}</h2></div><button ref={extraCloseRef} type="button" aria-label="Close Extra Video" onClick={() => { setActiveExtraVideo(null); extraLauncherRef.current?.focus(); }}><X /></button></header><div><NativeVideoPlayer video={activeExtraVideo.video} src={(classroom?.delivery.assetUrl || publishedUnitExtraVideoUrl)(publication, activeExtraVideo.video.asset)} autoPlayAttemptKey={activeExtraVideo.id} ariaLabel={`${activeExtraVideo.title} Extra Video player`} /></div></section></div> : null}
        <div style={{ display: "contents" }} inert={wordlist.open}><BookUnitExtraAudiosForPublication publication={publication} assetUrl={classroom?.delivery.assetUrl} unitNumber={unit?.number} pageId={page?.id} hidden={activityActive || Boolean(activeExtraVideo)} /></div>
      </div>

      {wordlist.supported && wordlist.state !== "ready" && <div className="word-list-status" role={wordlist.state === "error" ? "alert" : "status"}>{wordListReason}{wordlist.state === "error" && <button type="button" onClick={wordlist.retry}>Retry Word List</button>}</div>}
      <TeacherBookNavigation
        modalActionId={wordlist.open ? "vocabulary" : null}
      ...
      <ClassroomToolbar surfaceKey={classroomSurfaceKey} inert={wordlist.open} />
    </section>
  );
}
