...
export function NativeDragDropEditor({ compositeBinding = null, bookSlug, componentSlug, activityId, placementLabel, onDirtyChange = () => {}, onSaved = () => {} }) {
  ...
  const addTarget = (area) => {
    ...
    const word = interaction.words.find((entry) => entry.reusable || !used.has(entry.id)) || interaction.words[0];
  ...
  const setReusable = (wordId, reusable) => {
    const mappedCount = teacherDraft.parts[0].solution.mappings.filter((mapping) => nativeDragDropMappingWordIds(mapping).includes(wordId) && interaction.panels.flatMap((entry) => entry.dropTargets).find((target) => target.id === mapping.targetId)?.answerMode !== "any").length;
  ...
  const setMappingWords = (targetId, rawWordIds) => {
    ...
    const conflict = wordIds.map((wordId) => ({ wordId, mapping: teacherDraft.parts[0].solution.mappings.find((entry) => entry.targetId !== targetId && nativeDragDropMappingWordIds(entry).includes(wordId)) })).find(({ wordId, mapping }) => mapping && !reusableWordIds.has(wordId) && interaction.panels.flatMap((entry) => entry.dropTargets).find((target) => target.id === targetId)?.answerMode !== "any" && interaction.panels.flatMap((entry) => entry.dropTargets).find((target) => target.id === mapping.targetId)?.answerMode !== "any");
    if (conflict) {
    ...
    mutatePair((nextPublic, nextTeacher) => {
      ...
      target.capacity = target.answerMode === "any" ? 1 : Math.max(1, wordIds.length);
    });
  };
  ...
  return <section className="native-activity-foundation native-drag-drop-editor studio-editor">
    ...
    {tab === "content" ? <div className="studio-content-panel native-drag-drop-content">
      ...
      <fieldset className="native-drag-drop-layout-settings"><legend>Activity layout</legend>
        <label><input type="radio" name={`${activityId}-${interaction.panels[0]?.id || "empty"}-layout`} checked={interaction.layoutMode === "standard"} onChange={() => setLayoutMode("standard")} /> Standard drag-and-drop</label>
        <label><input type="radio" name={`${activityId}-${interaction.panels[0]?.id || "empty"}-layout`} checked={interaction.layoutMode === "text"} onChange={() => setLayoutMode("text")} /> Text drag-and-drop</label>
    ...
    {tab === "layout" ? <><NativeDragDropHotspotBulkImporter hasExistingTargets={interaction.panels.some((entry) => entry.dropTargets.length)} onPreview={previewHotspotImport} onApply={applyHotspotImport} /><div className="native-drag-drop-editor-grid">
      ...
      <aside className="native-drag-drop-editor-inspector">
        ...
        {selectedTarget ? <>
          ...
          <StudioField label="Target answer mode"><select aria-label="Target answer mode" value={selectedTarget.answerMode || "all"} onChange={(event) => mutatePublic((next) => { const target = next.parts[0].interaction.panels.flatMap((entry) => entry.dropTargets).find((entry) => entry.id === selectedTarget.id); target.answerMode = event.target.value; target.capacity = target.answerMode === "any" ? 1 : Math.max(1, (mappings.get(target.id) || []).length); })}><option value="all">Multiple slots: all selected answers</option><option value="any">One slot: any selected answer</option></select></StudioField>
          <label><input type="checkbox" checked={selectedTarget.sentenceStart === true} onChange={(event) => mutatePublic((next) => { next.parts[0].interaction.panels.flatMap((entry) => entry.dropTargets).find((entry) => entry.id === selectedTarget.id).sentenceStart = event.target.checked; })} />Sentence start: capitalize initial letter in Teacher view</label>
