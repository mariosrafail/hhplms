...
export function normalizeMarkWordsMarker(value, assets, { preset = false } = {}) {
  ...
  if (!["graphic", "outline", "underline"].includes(value.kind) || !/^#[0-9a-f]{6}$/i.test(value.color) || !["bottom", "manual"].includes(value.alignment)) throw new Error("Invalid marker style.");
