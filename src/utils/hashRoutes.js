...
function parseCourseRoute(hashView) {
  ...
  if ((subview === "pages" || subview === "flipbook") && parts[5]) {
    ...
    const publishedPageId = parts.length === 6 ? publishedManagedPageId(packageSlug, getComponentRouteSlug(component || {}), parts[5]) : null;
    if (managedRouteContract(packageSlug, getComponentRouteSlug(component || {})) && !publishedPageId) return null;
...
function parseTeacherRoute(hashView) {
  ...
  if (section === "books") {
    ...
    const publishedPageId = parts.length === 7 && (subview === "pages" || subview === "flipbook") ? publishedManagedPageId(packageSlug, getComponentRouteSlug(component || {}), parts[6]) : null;
    if ((subview === "pages" || subview === "flipbook") && parts[6] && managedRouteContract(packageSlug, getComponentRouteSlug(component || {})) && !publishedPageId) return null;
