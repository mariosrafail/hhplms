...
const inheritedKind = new URLSearchParams(location.search).get("inherited-marker");
const { id: inheritedId, ...inheritedMarker } = nativeMarkerCandidate().publicDocument.parts[0].interaction.presentation.markerPresets.find((preset) => preset.kind === inheritedKind) || {};
function Fixture() {
 ...
 return mode === "hotspot-editor" ? <NativeAudioTextHotspotEditor bookSlug="ultimate-b2" componentSlug="ultimate-b2-students-book" activityId={pair.publicDocument.activityId} publicDraft={pair.publicDocument} mutatePublic={mutatePublic} previewUrl={assetUrl} onIncompleteChange={() => {}} onStatusChange={() => {}} /> : mode === "editor" ? <NativeMarkWordsVisualEditor drawingDefaults={inheritedKind ? { correct: true, marker: inheritedMarker } : null} bookSlug="ultimate-b2" componentSlug="ultimate-b2-students-book" activityId={pair.publicDocument.activityId} publicDraft={pair.publicDocument} teacherDraft={pair.teacherDocument} {...{ mutatePair, mutatePublic, assetUrl }} onMessage={() => {}} onUploading={() => {}} isActive={() => true} /> : <div style={{ width: 1024, height: 582, transform: `scale(${scale})`, transformOrigin: "top left" }}><NativeReadableTextPresentation key={version} document={pair.publicDocument} assetUrl={assetUrl}>{(presentation, audioHotspotPresentation) => mode === "teacher" ? <NativeMarkWordsTeacherSurface {...pair} {...{ assetUrl, presentation, audioHotspotPresentation }} /> : <NativeMarkWordsStudentSurface document={pair.publicDocument} {...{ assetUrl, responses, audioHotspotPresentation }} onResponsesChange={setResponses} readOnly={mode === "review"} />}</NativeReadableTextPresentation></div>;
}
createRoot(document.getElementById("root")).render(new URLSearchParams(location.search).has("parent-editor") ? <NativeMarkWordsEditor bookSlug="ultimate-b2" componentSlug="ultimate-b2-students-book" activityId={nativeMarkerCandidate().publicDocument.activityId} /> : <Fixture />);
