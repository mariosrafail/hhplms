...
export async function verifyNativeMarkerAssignmentBrowser({ pool, token, assignmentId, pair }) {
 ...
 try {
  ...
  page.on("response", async (response) => { if (response.url().includes("book-content") && response.headers()["content-type"]?.includes("json")) payloads.push(await response.text()); });
